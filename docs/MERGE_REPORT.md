...
Sprint 4 membuat ulang tabel yang sama dari Sprint 3 tanpa `DROP POLICY IF EXISTS` akan gagal saat migration dijalankan. **Semua** `CREATE POLICY` dan
...
- `settings/account/page.tsx` + `AccountSettingsClient.tsx` export 1 klik,
  hapus akun sengaja _high-friction_ (ketik ulang frasa konfirmasi)
